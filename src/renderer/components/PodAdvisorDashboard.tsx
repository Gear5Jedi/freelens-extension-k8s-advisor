...
export const PodAdvisorDashboard = () => {
  ...
  if (selectedPod) {
    ...
    // Mock changes
    changes = ["[Demo Data] Pod was restarted due to a rollout 191 mins ago."];
  }
  ...
  return (
    <div className="pa-dashboard">
      ...
      {/* Content */}
      {!selectedPod ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "var(--pa-text-muted)", gap: "16px", padding: "48px 0" }}>
          <Shield size={48} color="var(--pa-border)" />
          <h2 style={{ margin: 0, color: "var(--pa-text-secondary)" }}>No Pod Selected</h2>
          <p>Please select a namespace and a pod from the dropdowns above to view its analysis.</p>
        </div>
      ) : activeTab === "Overview" ? (
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "var(--pa-text-muted)", gap: "16px", padding: "48px 0" }}>
          <div className="pa-pill pa-pill-info" style={{ fontSize: "0.8rem", padding: "4px 8px" }}>Coming Soon</div>
          <p>The <strong>{activeTab}</strong> view is currently under development.</p>
        </div>
      )}
    </div>
  );
